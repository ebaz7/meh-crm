...
export const updateExitPermitStatus = async (id: string, status: ExitPermitStatus, approverUser: User, extra?: { rejectionReason?: string, exitTime?: string }): Promise<ExitPermit[]> => {
    ...
    if(permit) {
        ...
        // مرحله ۳: انتظامات (ثبت ساعت خروج)
...
// ... سایر متدها (انبار، گفتگو، بازرگانی، انتظامات) بدون هیچگونه تغییر ...
...
export const getNextBijakNumber = async (company: string): Promise<number> => { try { const response = await apiCall<{ nextNumber: number }>(`/next-bijak-number?company=${encodeURIComponent(company)}`); return response.nextNumber; } catch(e) { const settings = await getSettings(); return (settings.warehouseSequences?.[company] || 1000) + 1; } };
export const getMessages = async (): Promise<ChatMessage[]> => { return await apiCall<ChatMessage[]>('/chat'); };
export const sendMessage = async (msg: ChatMessage): Promise<ChatMessage[]> => { return await apiCall<ChatMessage[]>('/chat', 'POST', msg); };
export const deleteMessage = async (id: string): Promise<ChatMessage[]> => { return await apiCall<ChatMessage[]>(`/chat/${id}`, 'DELETE'); };
export const updateMessage = async (msg: ChatMessage): Promise<ChatMessage[]> => { return await apiCall<ChatMessage[]>(`/chat/${msg.id}`, 'PUT', msg); };
...
export const restoreSystemData = async (fileData: string): Promise<{ success: boolean }> => { return await apiCall<{ success: boolean }>('/full-restore', 'POST', { fileData }); };
...
export const getNextTrackingNumber = async (): Promise<number> => { try { const response = await apiCall<{ nextTrackingNumber: number }>('/next-tracking-number'); return response.nextTrackingNumber; } catch(e) { const settings = await getSettings(); return (settings.currentTrackingNumber || 1000) + 1; } };
