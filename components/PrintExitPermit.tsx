      ...
      <div id={embed ? `print-permit-${permit.id}` : "print-area-exit"} 
        ...
        style={{ direction: 'rtl', width: '209mm', height: '147mm', margin: '0 auto', padding: '8mm', boxSizing: 'border-box' }}>
            ...
            <div className="mt-4 pt-2 border-t-2 border-black grid grid-cols-4 gap-2 text-center relative z-10">
                <div className="flex flex-col items-center justify-end h-20"><div className="mb-1 font-bold text-xs">{permit.requester}</div><div className="text-[9px] text-gray-500 border-t w-full pt-1">درخواست کننده</div></div>
                <div className="flex flex-col items-center justify-end h-20"><div className="mb-1">{permit.approverCeo ? <Stamp title="مدیر عامل" name={permit.approverCeo} /> : <span className="text-gray-300 text-[9px]">عدم تایید</span>}</div><div className="text-[9px] text-gray-500 border-t w-full pt-1">مدیر عامل</div></div>
                <div className="flex flex-col items-center justify-end h-20"><div className="mb-1">{permit.approverFactory ? <Stamp title="مدیر کارخانه" name={permit.approverFactory} /> : <span className="text-gray-300 text-[9px]">عدم تایید</span>}</div><div className="text-[9px] text-gray-500 border-t w-full pt-1">مدیر کارخانه</div></div>
                <div className="flex flex-col items-center justify-end h-20"><div className="mb-1">{permit.approverSecurity ? <Stamp title="انتظامات (خروج)" name={permit.approverSecurity} time={permit.exitTime} /> : <span className="text-gray-300 text-[9px]">عدم تایید</span>}</div><div className="text-[9px] text-gray-500 border-t w-full pt-1">انتظامات</div></div>
            </div>
        </div>
  );
