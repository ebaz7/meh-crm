...
const SecurityModule: React.FC<Props> = ({ currentUser }) => {
    ...
    const handleSupervisorDailySubmit = async () => {
        ...
        // Fix: Explicitly use string generic for Set and type date in forEach to prevent "unknown" index errors
        const datesToApprove = new Set<string>(readyDelays.map(d => d.date));
        if (settings) {
            // Fix: Explicitly type newMeta as a Record with string keys and DailySecurityMeta values
    ...
    const handleFactoryDailySubmit = async () => {
        ...
        // 2. Update Meta
        // Fix: Explicitly use string generic for Set
        const logDates = new Set<string>(readyLogs.map(l => l.date));
        const delayDates = new Set<string>(readyDelays.map(d => d.date));
        
        if (settings) {
            // Fix: Explicitly type newMeta to prevent index type unknown error
            let newMeta: Record<string, DailySecurityMeta> = { ...(settings.dailySecurityMeta || {}) };
            logDates.forEach((date: string) => { 
                // Fix: Explicitly typed date in forEach to avoid unknown error
                newMeta[date] = { ...newMeta[date], isFactoryDailyApproved: true }; 
            });
            delayDates.forEach((date: string) => { 
                // Fix: Explicitly typed date in forEach to avoid unknown error
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-orange-50 p-3 rounded-lg border border-orange-100">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button onClick={() => setMyName('evening')} className="text-[10px] bg-white border border-blue-200 text-blue-600 px-2 py-1 rounded flex items-center gap-1 hover:bg-blue-50"><UserIcon size={10}/> نام من</button>
                                </div>
                                <div className="flex gap-2">
                            ...
                            <div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button onClick={() => setMyName('night')} className="text-[10px] bg-white border border-indigo-200 text-blue-600 px-2 py-1 rounded flex items-center gap-1 hover:bg-blue-50"><UserIcon size={10}/> نام من</button>
                                </div>
                                <div className="flex gap-2">
                ...
                {activeTab === 'cartable' && (
                    <div className="p-4">
                        ...
                        {(currentUser.role === UserRole.SECURITY_HEAD || (permissions && permissions.canApproveSecuritySupervisor)) && getCartableItems().filter(i => i.type === 'delay' && i.status === SecurityStatus.APPROVED_SUPERVISOR_CHECK).length > 0 && (
                            <button onClick={handleSupervisorDailySubmit} className="w-full bg-blue-600 text-white px-6 py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-blue-700 shadow-md mb-6 transition-transform hover:scale-[1.01]">
                        ...
                        {getArchivedItems().map((item, idx) => {
                            ...
                            <div key={idx} className="border p-3 rounded-lg flex justify-between items-center bg-gray-50">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {canDelete(item) && <button onClick={() => handleDeleteItem(item.id, item.type)} className="text-red-500 hover:text-red-600 p-1"><Trash2 size={16}/></button>}
                        ...
                        {activeTab === 'delays' && (
                            <form className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div><label className="text-xs font-bold block mb-1">میزان تاخیر</label><input className="w-full border rounded p-2 text-center" value={delayForm.delayAmount || ''} onChange={e=>setDelayForm({...delayForm, delayAmount: e.target.value})} onKeyDown={handleKeyDown}/></div>
                                </div>
                                <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
