...
const ManageOrders: React.FC<ManageOrdersProps> = ({ orders, refreshData, currentUser, initialTab = 'current', settings, statusFilter }) => {
  ...
  const handleApprove = async (order: PaymentOrder) => {
      // Handle Void Approval Separately
      if (order.status === OrderStatus.PENDING_CANCELLATION) {
          if(window.confirm('آیا ابطال نهایی این دستور را تایید می‌کنید؟')) {
  ...
  const handleVoidRequest = async (order: PaymentOrder) => {
      if (window.confirm('آیا از درخواست ابطال این دستور اطمینان دارید؟ (پس از تایید مدیرعامل، سند باطل خواهد شد)')) {
          setProcessingId(order.id);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 h-full flex flex-col animate-fade-in relative">
        ...
        {selectedOrder && (
            <PrintVoucher 
                ...
                onEdit={(permissions?.canEditAll || (permissions?.canEditOwn && selectedOrder.requester === currentUser.fullName && selectedOrder.status !== OrderStatus.APPROVED_CEO && selectedOrder.status !== OrderStatus.VOIDED)) ? () => { setEditingOrder(selectedOrder); setSelectedOrder(null); } : undefined}
            />
        )}
                            ...
                            <tr key={order.id} className="hover:bg-blue-50/30 transition-colors group">
                                ...
                                <td className="p-4 text-center">
                                    <div className="flex items-center justify-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                        ...
                                        {/* VOID REQUEST BUTTON (For Requester if Rejected) */}
                                        {order.status === OrderStatus.REJECTED && order.requester === currentUser.fullName && (
                                        ...
                                        {/* VOID APPROVE BUTTON (For CEO if Pending Cancellation) */}
                                        {order.status === OrderStatus.PENDING_CANCELLATION && (currentUser.role === UserRole.CEO || currentUser.role === UserRole.ADMIN) && (
                                            <button onClick={() => handleApprove(order)} className="p-1.5 text-red-600 bg-red-50 hover:bg-red-100 rounded transition-colors border border-red-200" title="تایید ابطال نهایی"><XCircle size={16}/></button>
                                        )}

                                        {canApprove(order) && order.status !== OrderStatus.PENDING_CANCELLATION && (
                                            <button onClick={() => handleApprove(order)} className="p-1.5 text-green-600 bg-green-50 hover:bg-green-100 rounded transition-colors" title="تایید"><CheckCircle size={16} /></button>
                                        )}
