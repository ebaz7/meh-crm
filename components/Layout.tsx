...
interface LayoutProps {
  ...
  removeNotification: (id: string) => void; // New prop for individual removal
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, currentUser, onLogout, notifications, clearNotifications, removeNotification, onAddNotification }) => {
  ...
  useEffect(() => {
    ...
    // Logic for closing dropdown when clicking outside is now handled manually
  }, []);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => { 
      ...
      try { 
          ...
          window.location.reload();
      } catch (err) { alert('خطا در بروزرسانی اطلاعات'); } 
  };
  ...
  // REVISED NOTIFICATION DROPDOWN (Fixed Positioning to prevent hidden overflow)
  const NotificationDropdown = () => ( 
      <div 
        className="fixed z-50 bg-white rounded-xl shadow-2xl border border-gray-200 text-gray-800 overflow-hidden flex flex-col animate-fade-in"
        style={{
            bottom: window.innerWidth < 768 ? '80px' : 'auto', // Above bottom nav on mobile
            top: window.innerWidth >= 768 ? '60px' : 'auto', // Below header on desktop
            left: window.innerWidth < 768 ? '10px' : '260px', // Right side of sidebar on desktop
      ...
      >
          <div className="bg-blue-50 p-3 flex justify-between items-center border-b border-blue-100">
              <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-blue-800">اعلان‌ها:</span>
              </div>
              <div className="flex gap-2">
                  <button onClick={handleToggleNotif} className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${notifEnabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          ...
          <div className="bg-gray-50 p-2 flex justify-between items-center border-b">
              ...
              {notifications.length > 0 && (<button onClick={clearNotifications} className="text-gray-400 hover:text-red-500 flex items-center gap-1 text-[10px]"><Trash2 size={12} /> پاک کردن همه</button>)}
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
              {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                      <div 
                        key={n.id} 
                        className="p-3 border-b hover:bg-gray-50 text-right last:border-0 cursor-pointer transition-colors"
                        onClick={() => removeNotification(n.id)} // Remove on click
                        title="کلیک برای حذف"
                      >
  ...
  return (
    <div className="flex min-h-[100dvh] bg-gray-50 text-gray-800 font-sans relative">
      ...
      {/* GLOBAL DROPDOWN RENDER - Outside fixed containers */}
      ...
      {isUpdateAvailable && (
          <div className="fixed top-0 left-0 right-0 bg-blue-600 text-white z-[9999] p-3 text-center shadow-lg animate-slide-down flex justify-center items-center gap-4">
              ...
              <button onClick={handleReload} className="bg-white text-blue-600 px-4 py-1 rounded-full text-xs font-bold hover:bg-blue-50 transition-colors shadow-sm">بروزرسانی (رفرش)</button>
          </div>
      )}
              ...
              <div className="bg-white w-full max-w-sm rounded-t-2xl md:rounded-2xl p-6 shadow-2xl relative" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                          برای نصب اپلیکیشن، دکمه <span className="inline-block mx-1"><Share size={16}/></span> (اشتراک‌گذاری) در پایین مرورگر سافاری را بزنید و سپس گزینه <span className="font-bold text-gray-800">Add to Home Screen</span> را انتخاب کنید.
                      </p>
                      <div className="w-full bg-gray-100 rounded-xl p-4 flex items-center justify-center gap-4 mb-4">
                          <span className="text-xs font-mono text-gray-400">1. Share</span>
                          <ChevronRight size={16} className="text-gray-400"/>
                          <span className="text-xs font-bold text-gray-700">2. Add to Home Screen</span>
                      </div>
                      <button onClick={() => setShowIOSPrompt(false)} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold">متوجه شدم</button>
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="fixed bottom-24 left-4 md:bottom-8 md:left-8 z-[60]">
          <button onClick={() => setShowVoiceModal(true)} className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all flex items-center justify-center group" title="دستیار صوتی"><Mic size={24} className="group-hover:animate-pulse"/></button>
      </div>
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
                      ...
                      <form onSubmit={handleUpdateProfile} className="space-y-5">
                          ...
                          <div className="pt-2"><button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2"><Save size={18} /> ذخیره تغییرات</button></div>
                      </form>
                  </div>
              </div>
          </div>
      ...
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden relative min-w-0">
        <header className="bg-white shadow-sm p-4 md:hidden no-print flex items-center justify-between shrink-0 relative z-40 safe-pt">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="relative"><button onClick={() => setShowNotifDropdown(!showNotifDropdown)} className="relative p-2 rounded-full hover:bg-gray-100"><Bell size={20} className="text-gray-600" />{unreadCount > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border border-white"></span>}</button></div>
            </div>
        </header>
