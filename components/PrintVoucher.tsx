...
const PrintVoucher: React.FC<PrintVoucherProps> = ({ order, onClose, settings, onApprove, onReject, onEdit, embed }) => {
  ...
  // --- RENDER ---
  const content = (
      <div id={printAreaId} className="printable-content bg-white w-full max-w-[210mm] h-auto min-h-[148mm] mx-auto p-6 shadow-2xl rounded-sm relative text-gray-900 flex flex-col justify-between overflow-hidden print:shadow-none print:w-full" style={{ direction: 'rtl' }}>
        {/* Rejected Watermark */}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col items-center justify-start md:justify-center p-4 overflow-y-auto animate-fade-in safe-pb">
      <div className="relative md:absolute md:top-0 md:left-0 md:right-0 p-4 flex justify-between items-start z-50 no-print w-full md:w-auto mb-4 md:mb-0 order-1">
         <div className="bg-white p-3 rounded-xl shadow-lg flex flex-col gap-3 w-full md:max-w-lg mx-auto relative border border-gray-200">
             ...
             <div className="grid grid-cols-4 gap-2 relative">
                 ...
                 <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg flex items-center justify-center gap-1 text-xs font-bold transition-colors shadow-sm"><Printer size={14} /> چاپ</button>
      ...
      <div className="order-2 w-full flex justify-center">{content}</div>
    </div>
  );
};
